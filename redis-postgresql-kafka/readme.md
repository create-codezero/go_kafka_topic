...
1. **Open a secondary WSL terminal window.**
2. **Fire a single transactional payload into the stream:**

```bash
kcat -P -b localhost:9092 -t financial-transactions -X queue.buffering.max.ms=1
```

3. **Paste the following schema-compliant JSON line and hit ENTER:**
...
---
*Built with ❤️ by AuraStream Team*
```